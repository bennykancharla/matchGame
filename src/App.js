...
class App extends Component {
  state = {
    ...
    finalImageUrl: imagesList[0].imageUrl,
    timeInSec: 60,
    isGameRunning: true,
  }
  ...
  onClickImageItem = id => {
    const {finalImageUrl} = this.state
    const testingItem = imagesList.filter(
      each => finalImageUrl === each.imageUrl,
    )
    // console.log(testingItem)
    const randomImageUrl =
      imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
    if (id === testingItem[0].id) {
      this.setState(prev => ({
        finalImageUrl: randomImageUrl,
        score: prev.score + 1,
      }))
    } else {
  ...
  startTimer = () => {
    this.timeInterval = setInterval(() => {
      ...
      if (timeInSec > 0) {
        this.setState(prev => ({timeInSec: prev.timeInSec - 1}))
      } else {
        this.clearTimer()
        this.setState({isGameRunning: false})
      }
  ...
  render() {
    ...
    // const thumbnail = this.getThumbnail()[0]
    // console.log(thumbnail)
    return (
      <>
        <Header score={score} timeInSec={timeInSec} />
            ...
            <>
              ...
              <ul className="images-list-container">
                {imagesList.map(eachImage => {
                  if (eachImage.category === category) {
                    return (
                      <ImageItem
                        key={eachImage.id}
